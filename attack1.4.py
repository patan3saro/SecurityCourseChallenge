...
f = open("C:\\Users\\Utente\\Desktop\\Top353Million-probable-v2.txt.000.000.txt","r",errors='ignore')
content = f.readlines()
...
f2.close()
#operzaioni preliminari per fare una lista di bytes
j=0
for var in content[19048471:]3479489:
    #ho eliminato la condizione che la chiave possibile 
    # sia lunga 9 caratteri(9 perché c'è il \n) 
    if(len(var)>=9):
        if(len(bytes(var[0:8], encoding= 'utf-8'))==8):#verifico la lunghezza in 
         ...
         j=(j+1)#stampa tutto quello che soddisfa la condizione
         print(j)
